...
@section('content')   
     <div class="row">
        ...
        <div class="panel panel-green">
            ...
            <div class="navbar-right" style="margin-right:10px;margin-top:-50px;">
                ...
                <a href="{!! URL::route('admin.donhang.list') !!}" ><i class="btn btn-default" >Hoàn tất</i></a>
                                        ...
                                        @foreach ($chitiet as $val)
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    ?>
                                                    <a href="{!! URL::route('admin.donhanglo.list', [$sp->id] ) !!}">
                                                    {{$sp->sanpham_ten}}
                                                </a>
                                                </td>
    ...
    @foreach ($chitiet as $val)
      <?php  
          ...
          $ctl = DB::table('chitietdonhang')->where('sanpham_id',$val->sanpham_id)->first();
                        ...
                        <tr>
                            ...
                            @foreach($lohang as $lohang)
                                    ...
                                    ?>
                                        ...
                                        <td style="color: #fff;text-shadow: 0px -1px 4px white, 0px -2px 10px yellow, 0px -10px 20px #ff8000, 0px -18px 40px red;font: 20px 'BlackJackRegular';"><a href="{{URL::to('don-hang-chon-lo-san-pham/'.$lohang->id)}}">
                        ...
                        <tr>
                            ...
                            @foreach($lohang as $lohang)
                                    ...
                                    ?>
                                        ...
                                        <td>{{$lohang->lohang_so_luong_hien_tai}}</td>
                                        <td>
                                        @if($lohang->lohang_tinh_trang == 1)
                                        <h5 style="color:red">Hết hạn</h5>
                                        @else
                                        {{date('d-m-y',strtotime($ngaykt))}}
                                        @endif</td>
                                        <?php 
                                        $chitietlo = DB::table('cttheolo')->select('ctdonhang_id')->where('ctdonhang_id',$val->id)->first();
                                        ?>
                                        @if(!isset($chitietlo))
                                        <td style="color: #fff;text-shadow: 0px -1px 4px white, 0px -2px 10px yellow, 0px -10px 20px #ff8000, 0px -18px 40px red;font: 20px 'BlackJackRegular';"><a href="{{URL::to('don-hang-chon-lo-san-pham/'.$lohang->id)}}">
                                        <h4 > Thêm chi tiết bán theo lô</h4></a>
                                        </td>
                                        @else
                                        <td >
  ...
  <div class="col-sm-6">
  <section class="panel">
  <form role="form" method="post" action="{{URL::to('luu-don-hang-theo-lo')}}">
      {{ csrf_field() }}
      @if(session('chon'))
        <div class="panel panel-default">
          <div class="panel-heading" style="background:#a9d86e;">
        <h3 class="panel-title" style="color:black">Số lượng SP đã chọn theo lô: {{ count((array) session('chon')) }}</h3>
          </div>
        ...
        <table class="table table-hover">
                <thead>
                <tr>
                    <th class="product-name">Mã đơn</th>
                    <th class="product-name">Mã CT đơn</th>
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="cttma" value = "{!! $ctlb ->donhang_id!!}" class="form-control "   style="width:50px"/></td>
                            @foreach ($chitiet as $val)
                            <?php  
                                $sp = DB::table('sanpham')->where('id',$val->sanpham_id)->first();
                            ?>
                            @if($sp->id == $spid)
                            <td><input type="text" name="ctdonid" value = "{!! $val ->id!!}" class="form-control "   style="width:50px"/></td>
                            @endif
                            @endforeach
...
<script type="text/javascript">
        $(".update-cart").click(function (e) {
           ...
           var ele = $(this);
            ...
            $.ajax({
               url: '{{ url('cap-nhat-don-hang-theo-lo') }}',
               method: "patch",
